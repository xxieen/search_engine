...
void WebPageQuery::doQuery(const string &sought, vector<string> &retval) {
  ...
  if (!isExist) // 查询结果不存在
  {
    ...
    sort(queryResult.begin(), queryResult.end(),
         [](const WebPage &a, const WebPage &b) -> bool {
...
vector<double>
WebPageQuery::getQueryWordsWeightVector(const vector<string> &queryWords) {
  vector<double> queryWordsWeight;
  queryWordsWeight.reserve(queryWords.size());
  for (auto &word : queryWords) {
    // TF-IDF算法,计算搜索词的权重
    double TF =
        1; // 某个词在文章中出现的次数,将搜索词看成一篇文档的话那么搜索词至少出现一次
...
bool WebPageQuery::executeQuery(const vector<string> &queryWords,
  ...
  if (1 == queryWords.size()) // 查询单个词
  {
    ...
    for (int idx = 0; idx < queryResultSize; ++idx, ++iter) {
      queryResult.push_back(
          WebPage(iter->first, _offsetlib, queryWords, iter->second));
    }
  } else // 查询多个词
  {
    // 查找包含单个查询词的网页
    ...
    for (size_t idx = 1; idx < queryWords.size(); ++idx) {
      auto iter = std::set_intersection(
          webPages[0].begin(), webPages[0].end(), webPages[idx].begin(),
          webPages[idx].end(), webPages[0].begin());

...
KeyRecommander::KeyRecommander(const string &word) : _queryWord(word) {}
...
void KeyRecommander::queryIndexTable() {
  ...
  vector<string> queryWord =
      read_utf8_onebyone(_queryWord); // 将待查询的字符串每一个元素存入数组

  for (auto elem : queryWord) {
    if ((dicitionary->getIndexTable().find(elem)) !=
        (dicitionary->getIndexTable().end())) // 索引中存在待查寻元素
    {
      set<int> temp_line_no = dicitionary->getIndexTable().at(
          elem); // 找出每一个元素对应的行号set集合

      line_no.insert(temp_line_no.begin(),
                     temp_line_no.end()); // 并将集合插入到集合line_no中
    }
  }

  int i = 0; // 从0行开始遍历词典
  for (auto elem :
       dicitionary->getDict()) // 遍历词典 把行号set集合中 存在的行号
                               // 所对应的行中的词组存入_similarWord
  {
    if (line_no.count(i)) {
